 ...
 */
export function parseUploadResponse(xmlString) {
    const { data: doc, error } = tryCatch(() => {
        const grouped = create(xmlString).toObject({ group: true });
        const simple = create(xmlString).toObject();
 ...
 * @throws {AnafXmlParsingError} If XML cannot be parsed or has unexpected structure
 */
export function parseStatusResponse(xmlString) {
    const { data: doc, error } = tryCatch(() => {
        const grouped = create(xmlString).toObject({ group: true });
        const simple = create(xmlString).toObject();
        return { grouped, simple };
...
export function isErrorResponse(response) {
    return !!((response === null || response === void 0 ? void 0 : response.errors) ||
        ...
        (response === null || response === void 0 ? void 0 : response.executionStatus) === ExecutionStatus.Error ||
